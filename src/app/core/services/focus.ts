...
export class Focus {
  ...
  // 🚦 إشارات الحالة (Signals) - المصدر الوحيد للحقيقة في الواجهة
  elapsedSeconds = signal<number>(0);
  isActive = signal<boolean>(false);
  timerStatus = signal<'idle' | 'focusing' | 'break'>('idle');

  // 🚪 بيانات الغرفة الحالية (مزامنتها تضمن استقرار الداشبورد)
  currentRoomId = signal<string | null>(null);
  currentRoomName = signal<string | null>(null);

  // ⏱️ تتبع وقت البداية الفعلي (Timestamp)
  private startTime = signal<string | null>(null);
  private intervalId: any;

  // مفاتيح الذاكرة المحلية (Backup) لضمان عدم ضياع الجلسة عند التحديث
  ...
  constructor() {
    // 🛡️ استعادة الحالة الشاملة عند بداية تشغيل التطبيق
    this.restoreState();

    // 🔄 مزامنة تلقائية مع الـ LocalStorage في كل مرة تتغير فيها البيانات
    effect(() => {
      ...
      // حفظ القيم النشطة فقط لتوفير الذاكرة
  ...
  /**
   * 📺 تنسيق الوقت للعرض الاحترافي (00:00:00)
   * يتميز بكفاءة عالية في الرندر بفضل الـ Computed Signal
   */
   ...
   * يستخدم دالة rpc لضمان تحديث الستريك والوقت في عملية واحدة (Atomic)
   */
  async saveProgress(seconds: number) {
    ...
    try {
      ...
      this.reset(); // تصفير الحالة محلياً بعد النجاح
    } catch (err) {
  ...
  /**
   * ⏱️ إدارة تشغيل العداد
   */
  startTimer() {
    if (this.isActive() && this.timerStatus() === 'focusing') return;

    // تسجيل وقت البداية الحقيقي عند أول تشغيل لضمان مزامنة الزملاء
    if (!this.startTime()) {
  ...
  /**
   * 🧹 التصفير الشامل للحالة
   * يتم استدعاؤها عند إنهاء المذاكرة أو الخروج المتعمد من الغرفة
   */
  reset() {
    this.stopInterval();
    ...
    // تصفير بيانات الغرفة لضمان اختفاء كارت الجلسة من الداشبورد فوراً
  ...
  /**
   * 🏛️ تحديث بيانات الغرفة الحالية محلياً
   */
  ...
  /**
   * 📡 جلب وقت البداية الخام
   * ضروري جداً لمزامنة عدادات الزملاء في الغرفة بناءً على الوقت الحقيقي
   */
